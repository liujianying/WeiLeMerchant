...
public class PullToRefresh {
	...
	/**
	 * 下面几个参数的表达当前的几个状态，大致如下： NULL_STATE 空闲状态，即一般的状态 
	 * HEADER_UPDATING 头部正在刷新的状态
	...
	private ProgressBar imgHeaderArrow;// 头部的指示箭头
	private ProgressBar imgFooterArrow;// 底部的指示箭头
	...
	private boolean footerIsUsed = true;//设置是否使用上拉加载更多

	public PullToRefresh(ScrollView scrollAll, LinearLayout layoutIner, Context context) {
			
		this.scrollAll = scrollAll;
		this.layoutInner = layoutIner;
		footerIsUsed = true;
//		LogUitl.SystemOut("==PullToRefresh=====context======"+context);
	...
	private void initFooterView() {
		viewFooter = inflater.inflate(R.layout.head, null);
		imgFooterArrow = (ProgressBar) viewFooter.findViewById(R.id.head_arrowImageView);
		progressBarFooter = (ProgressBar) viewFooter.findViewById(R.id.head_progressBar);
		txtUpdatingFooter = (TextView) viewFooter.findViewById(R.id.head_tipsTextView);
		txtTimeFooter = (TextView) viewFooter.findViewById(R.id.head_lastUpdatedTextView);
		measureView(viewFooter);
		heightFooterView = viewFooter.getMeasuredHeight();
		LogUitl.LogE("heightFootView", "heightFootView=" + heightFooterView);
	...
	private void initHeaderView() {
		...
		imgHeaderArrow = (ProgressBar) viewHeader.findViewById(R.id.head_arrowImageView);
		progressBarHeader = (ProgressBar) viewHeader.findViewById(R.id.head_progressBar);
		txtUpdatingHeader = (TextView) viewHeader.findViewById(R.id.head_tipsTextView);
		txtTimeHeader = (TextView) viewHeader.findViewById(R.id.head_lastUpdatedTextView);
		// header layout
		measureView(viewHeader);
		heightHeaderView = viewHeader.getMeasuredHeight();
		LogUitl.LogE("heightHeaderView", "heightHeaderView=" + heightHeaderView);
		setHeaderPaddingTop(-1 * heightHeaderView);// 默认设置不可见，将刷新的接口提供给外部调用
		layoutInner.addView(viewHeader, 0);
	}
	
	public void setFooterIsUsed(boolean isUsed){
		footerIsUsed = isUsed;
	}
...
//		imgFooterArrow.clearAnimation();// 先清除动画，然后设置图片为null
	...
	public void setHeaderUpdatingComplete() {
		...
		imgHeaderArrow.setVisibility(View.VISIBLE);
		/*imgHeaderArrow.setImageResource(R.anim.head_pull_refresh);// 由于动画的原因，这里反而设置的图片是朝上
		imgHeaderArrow.setVisibility(View.VISIBLE);*/
	...
	public void setFooterUpdatingComplete() {
		setFooterPaddingBottom(-heightFooterView);//
//		imgFooterArrow.setVisibility(View.VISIBLE); 
//		imgFooterArrow.setImageResource(R.drawable.wb_refresh_arrow_down);// 和上面相反
		imgFooterArrow.setVisibility(View.VISIBLE);
...
//			}
	...
	public interface OnUpdatingListener {
		public void onHeaderUpdating(PullToRefresh puller);

		public void onFooterUpdating(PullToRefresh puller);
	}
	...
	private void setCurrentTime(TextView txt) {
		...
		txt.setText("更新于：" + dateFormat.format(new Date(System.currentTimeMillis())));
	}
	...
	/**
	 * 完成刷新，并且隐藏头部和尾部刷新View
	 */
	public void finshRefresh(){
		setHeaderUpdatingComplete();
		setFooterUpdatingComplete();
	}
	...
	static public void setGridViewHeightBasedOnChildren(GridView gridView) {
